...
function processCatalogRow(row, rowNumber) {
  ...
  const product = {
    ...
    canInstantTrade: row.canInstantTrade
  };
  ...
  if (row.canInstantTrade !== 'true' && row.canInstantTrade !== true && row.canInstantTrade !== '1') {
    logger.debug(`[CatalogSvc] Row #${rowNumber} PID ${product.pid} skipped: canInstantTrade is '${row.canInstantTrade}' (not true).`);
    return null;
  }
...
function transformBunjangRowToShopifyInput(bunjangProduct, shopifyPriceUsd) {
  ...
  logger.info(`[CatalogSvc] Using location ID from environment: ${locationId}`);
  ...
  // inventoryInfo에 환경변수의 locationId 포함
  const inventoryInfo = {
    quantity: 1,  // *** 항상 재고를 1로 설정 ***
    locationId: locationId  // 환경변수에서 가져온 위치 ID
  };
...
async function syncBunjangProductToShopify(bunjangProduct, jobId = 'N/A') {
  ...
  const lockKey = `sync-lock:${bunjangPid}`;
  const lockTTL = config.sync?.lockTTLMs || 60000; // 60초 기본값
  
  try {
    // 락 획득 시도
    if (redis) {
      const locked = await redis.set(lockKey, jobId, 'PX', lockTTL, 'NX');
      if (!locked) {
        logger.info(`[CatalogSvc:Job-${jobId}] Product ${bunjangPid} is already being processed by another worker`);
        return { status: 'skipped_locked', message: 'Already being processed' };
      }
      logger.debug(`[CatalogSvc:Job-${jobId}] Acquired lock for product ${bunjangPid}`);
    }
    
    // 데이터베이스 레벨 동시성 제어
    const processingResult = await SyncedProduct.startProcessing(bunjangPid, jobId, 30); // 30분 타임아웃
    if (!processingResult) {
      logger.info(`[CatalogSvc:Job-${jobId}] Product ${bunjangPid} is already being processed in database`);
      return { status: 'skipped_processing', message: 'Already being processed in database' };
    }
    ...
    if (syncedDoc.syncStatus === 'SYNCED' &&
      ...
      if (syncedDoc.shopifyGid) {
        try {
          const syncResult = await ensureInventoryIsOne(syncedDoc.shopifyGid, bunjangPid, jobId);
          if (!syncResult.success) {
            logger.warn(`[CatalogSvc:Job-${jobId}] Failed to ensure inventory is 1 for existing product ${bunjangPid}: ${syncResult.message}`);
          }
        } catch (err) {
          logger.warn(`[CatalogSvc:Job-${jobId}] Error ensuring inventory for existing product ${bunjangPid}: ${err.message}`);
        }
      }
      
      // 처리 완료 표시
      await SyncedProduct.completeProcessing(bunjangPid, jobId, true);
    ...
    try {
      ...
      if (operationType === 'update' && shopifyProductGid) {
        ...
        if (existingVariant && existingVariant.id) {
          try {
            ...
            // 4. 재고 수량 업데이트 - 환경변수의 위치 사용
            if (existingVariant.inventoryItem?.id) {
              try {
                const locationId = inventoryInfo.locationId;  // transformResult에서 가져온 위치 ID
                const inventoryItemId = existingVariant.inventoryItem.id;
                
                // 환경변수에서 지정한 위치에 재고 설정
                await shopifyService.updateInventoryLevel(inventoryItemId, locationId, 1);
                logger.info(`[CatalogSvc:Job-${jobId}] ✅✅ Inventory set to 1 at location ${locationId}`);
          ...
          try {
            ...
            if (firstVariant) {
              ...
              if (firstVariant.inventoryItem?.id) {
                const locationId = inventoryInfo.locationId;
                const inventoryItemId = firstVariant.inventoryItem.id;
                
                await shopifyService.updateInventoryLevel(inventoryItemId, locationId, 1);
                logger.info(`[CatalogSvc:Job-${jobId}] ✅✅ Set inventory to 1 at location ${locationId} for variant ${firstVariant.id}`);
              }
            } else {
        ...
        // variantInfo에 locationId 포함
        const variantInfo = {
          price: variantData.price, // 이미 문자열
          sku: variantData.sku,
          inventoryPolicy: variantData.inventoryPolicy,
          quantity: 1,  // *** 항상 재고를 1로 설정 ***
          locationId: inventoryInfo.locationId,  // 환경변수에서 가져온 위치 ID
        ...
        logger.debug(`[CatalogSvc:Job-${jobId}] Variant info with location:`, variantInfo);
        ...
        if (createdOrUpdatedProductId && shopifyApiResult.variants?.edges?.length > 0) {
          ...
          if (createdVariant.inventoryItem?.id) {
            try {
              ...
              logger.info(`[CatalogSvc:Job-${jobId}] ✅✅ Forced inventory to 1 at location ${inventoryInfo.locationId}`);
            } catch (invError) {
        ...
        if (finalVariant) {
          ...
          if (inventoryLevels.length > 0) {
            ...
            for (const edge of inventoryLevels) {
              ...
              // 환경변수 위치 확인
              if (location.id === inventoryInfo.locationId) {
                if (available === 1) {
                  logger.info(`[CatalogSvc:Job-${jobId}] ✅ Target location inventory correctly set to 1`);
                } else {
                  logger.error(`[CatalogSvc:Job-${jobId}] ❌ Target location inventory is ${available} instead of 1`);
                  
                  // 재고가 1이 아니면 다시 설정 시도
                  if (finalVariant.inventoryItem?.id) {
                    try {
                      await shopifyService.updateInventoryLevel(finalVariant.inventoryItem.id, inventoryInfo.locationId, 1);
      ...
      logger.info(`[CatalogSvc:Job-${jobId}] Successfully ${operationType}d Shopify product ${createdOrUpdatedProductId} for Bunjang PID ${bunjangPid}. Price: ${shopifyPriceString}, Inventory: 1 at location ${inventoryInfo.locationId}`);
      await SyncedProduct.completeProcessing(bunjangPid, jobId, true);
      return { status: 'success', operation: operationType, shopifyGid: createdOrUpdatedProductId };

    } catch (error) {
      logger.error(`[CatalogSvc:Job-${jobId}] Error syncing product ${bunjangPid}:`, error);
      await SyncedProduct.completeProcessing(bunjangPid, jobId, false);
      throw error;
    }
    ...
    if (redis) {
      try {
        ...
        logger.debug(`[CatalogSvc:Job-${jobId}] Released lock for product ${bunjangPid}`);
      } catch (lockError) {
        logger.warn(`[CatalogSvc:Job-${jobId}] Error releasing lock for product ${bunjangPid}:`, lockError);
      }
    }
  }
}
...
async function fetchAndProcessBunjangCatalog(catalogType, jobIdForLog = 'N/A') {
  ...
  bunjangProducts.forEach(product => {
    if (!uniqueProductsMap.has(product.pid)) {
      uniqueProductsMap.set(product.pid, product);
    } else {
      // 중복된 PID가 있는 경우, 더 최신 정보를 유지
      const existingProduct = uniqueProductsMap.get(product.pid);
      const existingUpdatedAt = existingProduct.updatedAt ? new Date(existingProduct.updatedAt).getTime() : 0;
      const newUpdatedAt = product.updatedAt ? new Date(product.updatedAt).getTime() : 0;
      
      if (newUpdatedAt > existingUpdatedAt) {
        uniqueProductsMap.set(product.pid, product);
        logger.debug(`[CatalogSvc:Job-${jobIdForLog}] Replaced duplicate PID ${product.pid} with newer data (${new Date(product.updatedAt).toISOString()})`);
      }
    }
  });
  
  const uniqueProducts = Array.from(uniqueProductsMap.values());
  const duplicateCount = bunjangProducts.length - uniqueProducts.length;
  
  if (duplicateCount > 0) {
    logger.info(`[CatalogSvc:Job-${jobIdForLog}] Removed ${duplicateCount} duplicate products. Processing ${uniqueProducts.length} unique products.`);
  }
  ...
  logger.info(`[CatalogSvc:Job-${jobIdForLog}] Processing ${uniqueProducts.length} unique items from Bunjang catalog...`);
  const concurrency = config.bunjang?.syncConcurrency || 1;
  const productChunks = [];
  for (let i = 0; i < uniqueProducts.length; i += concurrency) {
    productChunks.push(uniqueProducts.slice(i, i + concurrency));
  }
  ...
  for (const chunk of productChunks) {
    ...
    chunkResults.forEach(result => {
      if (result.status === 'fulfilled' && result.value) {
        ...
        else if (result.value.status === 'skipped_locked') skippedLockedCount++;
        else if (result.value.status === 'skipped_processing') skippedProcessingCount++;
    ...
    logger.debug(`[CatalogSvc:Job-${jobIdForLog}] Processed a chunk. Totals - Success: ${successfullyProcessed}, FilterSkip: ${skippedByFilterCount}, NoChangeSkip: ${skippedNoChangeCount}, LockedSkip: ${skippedLockedCount}, ProcessingSkip: ${skippedProcessingCount}, Errors: ${errorCount} / TotalUnique: ${uniqueProducts.length}`);
  }
  ...
  const summary = {
    ...
    totalValidProducts: bunjangProducts.length,
    duplicateProductsRemoved: duplicateCount,
    uniqueProductsToProcess: uniqueProducts.length,
    successfullyProcessed,
    errors: errorCount,
    skippedByFilter: skippedByFilterCount,
    skippedNoChange: skippedNoChangeCount,
    skippedLocked: skippedLockedCount,
    skippedProcessing: skippedProcessingCount,
  };
...
module.exports = {
  ...
  generateBunjangCatalogFilename,
  syncBunjangProductToShopify,
  transformBunjangRowToShopifyInput,
  processCatalogRow,
